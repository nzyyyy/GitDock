...
- 历史页可通过“提交”标题右侧的分支菜单搜索并选择本地或远程分支，同步筛选提交列表与 graph，包含完整祖先和合并历史，无需切换工作区；默认显示全部分支。
- 分支比较采用与 commit 一致的文件列表，显示文件名、路径、增删行数与文件总数；点击文件只读查看差异，返回列表继续选择，包含新增、删除、重命名、二进制与仅元数据变化；无差异时显示明确提示
- 支持文件历史与 Blame；加载、空结果和失败状态分别展示，切换仓库、工作流或文件时不会沿用旧检查结果
...
- 多选文件后通过下拉菜单批量暂存、取消暂存、丢弃或删除（含部分暂存文件）；工作区标题左侧的复选框可全选或取消选择已暂存、部分暂存、未暂存和未跟踪文件（包括折叠分组），标题与分组复选框均在部分选中时显示半选状态；冲突和已忽略文件不参与全选
- 在英文和简体中文界面之间切换，并记住语言选择
- 流畅滚动浏览跨页连续且窗口化渲染的提交拓扑图（不显示 stash 及其内部辅助提交，统一在 Stashes 面板管理）；提交后自动刷新图与提交列表，并可查看提交详情（元数据与变更文件列表）与单文件差异、执行 cherry-pick 与 revert
- 通过可折叠分组、置顶收藏组、新建空分组和拖拽排序管理仓库；状态色条与右上角数量标明工作区变更，并对所有已登记仓库随文件变化更新（不限于当前选中项），键盘操作可完成同组排序
- 搜索并分组查看本地与远程分支，当前本地分支始终置顶，远程仓库前缀与分支路径分层显示；将远程分支直接检出为本地分支，并创建、切换、合并、变基、重命名和删除分支
- History 提交以及 Branches 中的分支、标签和远程条目均可通过悬停显示的行尾三点菜单执行与右键菜单相同的操作
- 管理标签、远程仓库、stash 与 submodule，并可查看 stash 中完整的已跟踪与未跟踪文件变更及单文件差异
- Fetch、Pull、Push，以及带 lease 的强制推送；Push 在远程还没有对应分支时会创建该分支并设置 upstream；Pull/Push 采用主操作与下拉箭头相连的分裂按钮，直接显示待同步提交数，并分别收纳拉取策略与推送设置；Fetch / Pull / Push 执行中可悬停转圈并点击停止以取消；所有 Git 操作完成后显示短暂的结果提示
