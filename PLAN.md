...
- Interactive rebase for reorder, reword, squash, fixup, and drop. Trigger: ordinary rebase plus cherry-pick no longer covers common history-editing needs.
  - Implemented: reorder, reword, squash, fixup, and drop for the `onto..HEAD` range through a backend-owned todo plus `GIT_SEQUENCE_EDITOR`/`GIT_EDITOR` helper scripts; reword messages replay in todo order and squash keeps the auto-combined message; destructive preview; conflict continue/skip/abort reuse the existing rebase flow. Squash/fixup as the first step is rejected.
- File history and blame. Trigger: users need repository investigation inside GitDock rather than their editor.
  - Implemented: per-file history via `git log --follow` (capped at 500) with commit-scoped file diffs, and line blame via `git blame --porcelain` with hunk attribution. Entry points are changed-file rows and the diff header; files over 1 MiB or 20,000 lines are rejected rather than virtualized.
