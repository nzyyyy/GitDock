...
export function BlameView({ blame, path, loading, error, onBack }: { blame?: BlameFile; path?: string; loading?: boolean; error?: string; onBack: () => void }) {
  ...
  return <div className="blame-view">
    <header className="canvas-header"><button onClick={onBack}>← {t("back")}</button><strong>{t("blameOf")} {path ?? blame?.path}</strong></header>
    {loading ? <p role="status">{t("loading")}</p> : error ? <p role="alert">{error}</p> : !blame?.content.length ? <p role="status">{t("emptyFile")}</p> : <div className="blame-list">{blame.content.map((line, index) => {
