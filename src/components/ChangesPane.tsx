...
export function ChangesOverview({ repository, snapshot }: { repository?: RepositorySummary; snapshot?: WorkingTreeSnapshot }) {
  const { t } = useI18n();
  if (!snapshot) return <div className="canvas-empty" role="status"><p>{t("loading")}</p></div>;
  const changed = snapshot.files.filter((file) => !file.ignored).length;
...
export function ChangesPane({ repository, snapshot, onOpen, onOpenExternal, onRun, onFileHistory, onBlame }: { repository?: RepositorySummary; snapshot?: WorkingTreeSnapshot; onOpen: (file: FileChange, staged: boolean) => void; onOpenExternal: (path: string) => void; onRun: RunOperation; onFileHistory: (path: string) => void; onBlame: (path: string) => void }) {
  ...
  const validStagePaths = new Set(groups.filter(([, , type]) => type === "unstaged" || type === "untracked" || type === "mixed").flatMap(([, entries]) => entries.map((file) => file.path)));
  const validUnstagePaths = new Set(groups.filter(([, , type]) => type === "staged" || type === "mixed").flatMap(([, entries]) => entries.map((file) => file.path)));
  useEffect(() => {
  ...
  const selectableCount = validStagePaths.size + validUnstagePaths.size;
  const selectedCount = stageSelection.filter((path) => validStagePaths.has(path)).length + unstageSelection.filter((path) => validUnstagePaths.has(path)).length;
  const selectionEnabled = !!repository && !!snapshot && selectableCount > 0;
  const allSelected = selectionEnabled && selectedCount === selectableCount;
  const partiallySelected = selectionEnabled && selectedCount > 0 && !allSelected;
  const title = <label className="working-tree-title"><input type="checkbox" aria-label={`${t("selectAll")} ${t("workingTree")}`} disabled={!selectionEnabled} checked={allSelected} ref={(input) => { if (input) input.indeterminate = partiallySelected; }} onChange={() => {
    if (allSelected) clearSelection();
    else { setStageSelection([...validStagePaths]); setUnstageSelection([...validUnstagePaths]); }
  }} /><span>{t("workingTree")}</span></label>;
  if (!repository) return <div className="changes-pane"><div className="pane-title">{title}</div></div>;
  if (!snapshot) return <div className="changes-pane"><div className="pane-title">{title}</div><div className="change-groups" role="status" aria-label={t("loading")}><div className="skeleton-list">{[0, 1, 2, 3, 4].map((index) => <div className="skeleton-row" key={index} />)}</div></div><MemoCommitBox repositoryId={repositoryId} hasStagedChanges={false} onRun={onRun} /></div>;
  return <div className="changes-pane"><div className="pane-title">{title}<span className="batch-actions">{hasSelection && <><button disabled={!stageSelection.length} onClick={() => batch({ type: "stageFiles", paths: stageSelection })}>{t("stageSelected")} ({stageSelection.length})</button><RowMenu label={t("batchActions")} glyph="▾"><button disabled={!unstageSelection.length} onClick={() => batch({ type: "unstageFiles", paths: unstageSelection })}>{t("unstageSelected")} ({unstageSelection.length})</button><button disabled={!discardPaths.length} onClick={() => batch({ type: "discardTracked", paths: discardPaths })}>{t("discard")} ({discardPaths.length})</button><button disabled={!trashPaths.length} onClick={() => batch({ type: "trashUntracked", paths: trashPaths })}>{t("trash")} ({trashPaths.length})</button></RowMenu></>}</span></div><div className="change-groups">{repository?.ongoing && <div className="ongoing"><strong>{repository.ongoing.kind} {t("inProgress")}</strong>{repository.ongoing.canContinue && <button onClick={() => onRun({ type: "continue", kind: repository.ongoing!.kind })}>{t("continue")}</button>}{repository.ongoing.canSkip && <button onClick={() => onRun({ type: "skip", kind: repository.ongoing!.kind })}>{t("skip")}</button>}{repository.ongoing.canAbort && <button onClick={() => onRun({ type: "abort", kind: repository.ongoing!.kind })}>{t("abort")}</button>}</div>}{groups.map(([name, entries, type]) => {
    ...
    const onToggle = type === "mixed"
      ? (path: string) => { toggle(path, stageSelection, setStageSelection); toggle(path, unstageSelection, setUnstageSelection); }
      : (path: string) => toggle(path, selected, setSelected);
    const onSelectAll = type === "mixed"
      ? () => {
        const paths = entries.map((file) => file.path);
        const allOn = paths.every((path) => stageSelection.includes(path) && unstageSelection.includes(path));
        if (allOn) {
          setStageSelection((current) => current.filter((path) => !paths.includes(path)));
          setUnstageSelection((current) => current.filter((path) => !paths.includes(path)));
        } else {
          setStageSelection((current) => [...new Set([...current, ...paths])]);
          setUnstageSelection((current) => [...new Set([...current, ...paths])]);
        }
      }
      : () => setSelected(entries.every((file) => selected.includes(file.path)) ? selected.filter((path) => !entries.some((file) => file.path === path)) : [...new Set([...selected, ...entries.map((file) => file.path)])]);
    return <ChangeGroup key={type} name={name} files={entries} type={type} selected={selected} onToggle={onToggle} onSelectAll={onSelectAll} onOpen={onOpen} onOpenExternal={onOpenExternal} onRun={onRun} onFileHistory={onFileHistory} onBlame={onBlame} />;
  })}</div><MemoCommitBox repositoryId={repositoryId} hasStagedChanges={hasStagedChanges} onRun={onRun} /></div>;
}
...
function FileRowActions({ additions, deletions, children }: { additions: number | null; deletions: number | null; children?: React.ReactNode }) {
  const stats = additions != null || deletions != null;
  return <span className={`file-row-actions${stats && children ? "" : " always-ops"}`}>{stats && <span className="file-row-stats"><span className="commit-stat-add">+{additions ?? 0}</span><span className="commit-stat-del">−{deletions ?? 0}</span></span>}{children ? <span className="file-row-ops">{children}</span> : null}</span>;
}
