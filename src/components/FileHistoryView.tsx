...
export function FileHistoryView({ path, entries, selectedOid, diff, loading, error, onBack, onSelect }: {
  path: string;
  ...
  return <div className="file-history-view">
    ...
    {diff !== undefined && <pre className="diff-lines raw-diff">{diff || t("noDifferences")}</pre>}
  </div>;
}
