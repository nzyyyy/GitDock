...
export function ToastStack({ toasts, onDismiss }: { toasts: OperationToast[]; onDismiss: (id: number) => void }) {
  return <div className="toast-stack" aria-live="polite">{toasts.map((toast) => <OperationToastView key={toast.id} toast={toast} onDismiss={onDismiss} />)}</div>;
}

function OperationToastView({ toast, onDismiss }: { toast: OperationToast; onDismiss: (id: number) => void }) {
  const { t } = useI18n();
  useEffect(() => {
    if (toast.outcome === "failed") return;
    const timer = window.setTimeout(() => onDismiss(toast.id), 3_000);
    return () => window.clearTimeout(timer);
  }, [toast.id, toast.outcome, onDismiss]);
