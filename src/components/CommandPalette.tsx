...
export function CommandPalette({ items, onClose }: { items: CommandItem[]; onClose: () => void }) {
  ...
  }} /><div id="command-list" role="listbox">{visible.map((item, index) => <button id={`command-${item.id}`} role="option" aria-selected={index === active} className={index === active ? "active" : ""} key={item.id} onMouseEnter={() => setActive(index)} onClick={() => run(item)}>{item.label}</button>)}{visible.length === 0 && <p className="command-empty" role="status">{t("noMatchingCommands")}</p>}</div></dialog>;
}
