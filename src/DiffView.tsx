...
export function DiffView({ diff, snapshotId, mode, onModeChange, onBack, onRun, fileActions, onFileHistory, onBlame, caption }: { diff: DiffFile; snapshotId?: number; mode: DiffMode; onModeChange: (mode: DiffMode) => void; onBack: () => void; onRun: (request: OperationRequest) => void | Promise<void>; fileActions?: boolean; onFileHistory?: (path: string) => void; onBlame?: (path: string) => void; caption?: string }) {
  const { t } = useI18n();
  const file = useMemo(() => {
