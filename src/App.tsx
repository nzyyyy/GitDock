...
export default function App() {
  ...
  const resetHistoryDetail = useCallback(() => { workingTree.closeDiff(); fileInspection.close(); }, [workingTree.closeDiff, fileInspection.close]);
  const history = useHistory({ reportError, selectedIdRef, selectedId, tab, onReset: resetHistoryDetail });
  ...
  const pullLabel = `${t("pull")}${selected?.behind ? ` ↓${selected.behind}` : ""}`;
  const pushLabel = `${t("push")}${selected?.ahead ? ` ↑${selected.ahead}` : ""}`;
  const syncBusyIcon = <span className="sync-busy-icon" aria-hidden="true"><span className="sync-spinner" /><span className="sync-stop" /></span>;
  const syncProps = (kind: "fetch" | "pull" | "push", label: string, canRun: boolean, start: () => void) => {
    const busy = !!busySync[kind];
    return { "aria-label": busy ? `${t("cancel")} ${t(kind)}` : label, "aria-busy": busy, disabled: !canRun && !busy, onClick: () => busy ? cancelSync(kind) : start(), children: busy ? syncBusyIcon : label };
  };
  ...
  const outputPanel = <section className={`output-panel ${outputOpen ? "open" : ""}`}>
    <button className="output-handle" aria-expanded={outputOpen} onClick={() => setOutputOpen((value) => !value)}><span>{t("gitOutput")}</span><span>{busyOperations.length ? `${busyOperations.length} ${t("running")}` : `${logCount} ${t("lines")}`} {outputOpen ? "⌄" : "⌃"}</span></button>
  ...
  return (
    <I18nProvider language={language}><div className="app-shell" style={{ gridTemplateColumns: `${leftWidth}px 1fr` }}>
      <aside className="repo-sidebar">
        ...
        <div className="sidebar-search"><label className="search"><span aria-hidden="true">⌕</span><input name="repositorySearch" autoComplete="off" autoCapitalize="none" autoCorrect="off" spellCheck={false} aria-label={t("searchRepositories")} placeholder={t("findRepository")} value={filter} onChange={(event) => setFilter(event.target.value)} /></label><span className="sidebar-add"><RowMenu label={t("add")} glyph="+"><button onClick={register}>{t("addRepository")}</button><button onClick={clone}>{t("clone")}</button><button onClick={initialize}>{t("initialize")}</button><button onClick={() => showDialog({ title: t("addGroup"), fields: [{ name: "group", label: t("group"), required: true }], onSubmit: ({ group }) => addGroup(String(group).trim()) })}>{t("addGroup")}</button></RowMenu></span></div>
        <div className={`repo-list${draggingId ? " drag-active" : ""}`} role="list" aria-label={t("repositories")} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) clearRepositoryDropHint(); }} onDragEnd={finishRepositoryDrag}>
          {repositoryGroups.map((group) => <section role="group" aria-label={group.label} className={`repo-group ${!collapsedGroups.has(group.key) && !group.repositories.length ? "empty" : ""} ${dropHint?.groupKey === group.key ? "drop-target" : ""}`} key={group.key} onDragEnter={acceptRepositoryDrop} onDragOver={(event) => hintRepositoryDrop(event, group.key)} onDrop={(event) => { event.preventDefault(); flushSync(() => { dropRepository(group.key); finishRepositoryDrag(); }); }}>
          ...
          {filter.trim() && repositoryGroups.every((group) => !group.repositories.length) && <p className="pane-empty">{t("noMatchingRepositories")}</p>}
        </div>
        <div className="resize-handle resize-left" role="separator" tabIndex={0} aria-label={t("resizeRepositories")} aria-orientation="vertical" aria-valuemin={LAYOUT_LIMITS.left[0]} aria-valuemax={LAYOUT_LIMITS.left[1]} aria-valuenow={leftWidth} onKeyDown={(event) => resizeWithKeyboard("left", event)} onPointerDown={(event) => beginResize("left", event)} />
      </aside>
      ...
      <main className="workspace">
        <header className="topbar" data-tauri-drag-region="deep">
          ...
          <div className="sync-actions"><button aria-label={t("commandPalette")} onClick={() => setPaletteOpen(true)}>⌘K</button><button {...syncProps("fetch", t("fetch"), Boolean(selected?.capabilities.canManageRemotes), () => { void run({ type: "fetch", remote: null, prune: false }); })} /><div className={`sync-split${selected?.behind ? " sync-pending" : ""}`}><button className={`sync-button${selected?.behind ? " sync-pending" : ""}`} {...syncProps("pull", pullLabel, Boolean(selected?.capabilities.canWriteWorkTree), () => { void run({ type: "pull", strategy: null }); })} /><RowMenu label={`${t("pull")}: ${t("moreActions")}`} glyph={<svg className="sync-chevron" viewBox="0 0 8 6" aria-hidden="true"><path d="m1 1.5 3 3 3-3" /></svg>} disabled={!selected?.capabilities.canWriteWorkTree}><button onClick={() => run({ type: "pull", strategy: "merge" })}>{t("pullMerge")}</button><button onClick={() => run({ type: "pull", strategy: "rebase" })}>{t("pullRebase")}</button><button onClick={() => run({ type: "pull", strategy: "fastForwardOnly" })}>{t("pullFf")}</button></RowMenu></div><div className={`sync-split${selected?.ahead ? " sync-pending" : ""}`}><button className={`sync-button${selected?.ahead ? " sync-pending" : ""}`} {...syncProps("push", pushLabel, Boolean(selected?.capabilities.canManageRemotes), () => { void run({ type: "push", remote: null, branch: null }); })} /><RowMenu label={`${t("push")}: ${t("moreActions")}`} glyph={<svg className="sync-chevron" viewBox="0 0 8 6" aria-hidden="true"><path d="m1 1.5 3 3 3-3" /></svg>} disabled={!selected?.capabilities.canManageRemotes}><button onClick={forcePush}>{t("forcePush")}</button><button onClick={() => showDialog({ title: t("setUpstream"), fields: [{ name: "remote", label: t("remote"), value: "origin", required: true }], onSubmit: ({ remote }) => { if (selected?.branch) run({ type: "setUpstream", remote: String(remote).trim(), branch: selected.branch }); } })}>{t("setUpstream")}</button></RowMenu></div><RowMenu label={t("more")}><button onClick={refreshRepositories}>{t("refreshAll")}</button><button onClick={() => run({ type: "undoLastCommit" })}>{t("undoCommit")}</button><button onClick={() => showDialog({ title: t("renameEntry"), fields: [{ name: "name", label: t("repositoryName"), value: selected?.name, required: true }], onSubmit: ({ name }) => updateSelected({ name: String(name).trim() }) })}>{t("renameEntry")}</button><button onClick={relocateSelected}>{t("relocate")}</button><button onClick={selectGit}>{t("selectGit")}</button><button onClick={toggleLanguage}>{t("language")}</button><button className="menu-danger" onClick={removeSelected}>{t("removeGitDock")}</button></RowMenu></div>
        </header>
        ...
        <div id="workflow-panel" className="work-area" role="tabpanel" aria-labelledby={`workflow-tab-${tab}`} style={{ gridTemplateColumns: `minmax(480px, 1fr) ${rightWidth}px` }}>
          ...
          <aside className="tool-pane"><div className="resize-handle resize-right" role="separator" tabIndex={0} aria-label={t("resizeDetails")} aria-orientation="vertical" aria-valuemin={LAYOUT_LIMITS.right[0]} aria-valuemax={LAYOUT_LIMITS.right[1]} aria-valuenow={rightWidth} onKeyDown={(event) => resizeWithKeyboard("right", event)} onPointerDown={(event) => beginResize("right", event)} />
            ...
            {tab === "history" && <MemoHistoryPane key={selectedId} branches={history.branches} branchRef={history.branchRef} onBranchChange={history.selectBranch} commits={commits} selectedOid={selectedCommit} loading={historyLoading} hasMore={hasMore} onLoadMore={loadMoreHistory} onSelect={openCommit} onRun={run} />}
            ...
            {tab === "stashes" && selectedId && <MemoStashesPane repositoryId={selectedId} selectedOid={detailKind === "stash" ? selectedCommit : undefined} onSelect={openStash} onRun={run} onDialog={showDialog} onError={reportError} />}
          </aside>
        </div>
